...
async def session(request: Request, session_data: SessionRequest):
    ...
    check_prediction_intent = await asyncio.to_thread(
        predict_intent, session_data.user_query, 0.5
    )

    if (
        check_prediction_intent["score"] < 0.5
        or check_prediction_intent["intent"] == "out_of_domain"
    ):
        return {"message": Chatbot_Constants.OUT_OF_DOMAIN_RESPONSE}
    elif check_prediction_intent["intent"] == "greetings":
        return {
            "message": f"Hello {session_data.user_name} I am viridi ai, how can I help you ?"
        }
    else:
        # Call the chatbot service to do the query and send the response as stream
        query_response = await ChatService.get_query_result(
            session_data.user_query, session_data.user_id
        )
