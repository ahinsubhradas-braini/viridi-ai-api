...
class TemplateManager:
    ...
    def save_template(
        self, api_url: str, output_schema: Dict[str, Any], transformer_content: str
    ) -> str:
        """
        ...
        if self.template_exists(api_url, output_schema):
            ...
            try:
                ...
                transformer = agent.generate_transformer(
                    input_schema=input_data, output_schema=output_schema
                )
                ...
                logger.info(
                    f"Agent performance - Requests: {stats['total_requests']}, "
    ...
    def render_template(
        self, transformer_content: str, input_data: Dict[str, Any]
    ) -> str:
        """
    ...
    def validate_template(
        self, transformer_content: str, sample_data: Optional[Dict[str, Any]] = None
    ) -> bool:
        """
    ...
    def get_template_info(self, template_filename: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get template info for {template_filename}: {str(e)}"
            )
            return None
