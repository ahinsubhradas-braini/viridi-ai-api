...
async def transform_third_party_data(request: Request, body: TransformRequest,headers=Depends(hmac_headers)):
...
@limit_request("5/minute")
async def check_hmac(request:Request,body: CheckHmacRequest,headers=Depends(hmac_headers)):
